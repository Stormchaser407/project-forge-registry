...
def normalize_report_name(report_name: str) -> str:
    ...
    if candidate.name != report_name or report_name in {"", ".", ".."}:
        raise ValueError(
            "report name must be a simple filename inside this repository's artifacts directory"
        )
    return report_name
...
def ensure_destination_within_vault_root(
    destination: Path,
    ...
    if (
        ...
        raise ValueError(
            f"destination path must stay inside vault project root {resolved_root}"
        )
...
def load_passport_record(
    ...
    if record_slug != slug:
        raise ValueError(
            f"passport slug mismatch: expected '{slug}', found '{record_slug}'"
        )
    ...
    if any(
        ...
        raise ValueError(
            f"passport file has missing required string fields: {passport_path}"
        )
...
def destination_folder_name(record: ObsidianSyncPassportRecord) -> str:
    ...
    if not folder_name:
        raise ValueError(
            "passport obsidian path must include a final project folder name"
        )
    if folder_name in {".", ".."}:
...
) -> tuple[list[Path], list[ObsidianSyncExcludedFile]]:
    allowed: list[Path] = []
    ...
    for candidate in sorted(source_mirror_dir.rglob("*")):
        ...
        if suffix_lower in EXCLUDED_SUFFIXES:
            excluded.append(
                ObsidianSyncExcludedFile(
                    source_path=candidate,
                    reason=f"excluded_suffix={suffix_lower}",
                )
            )
            continue
        if suffix_lower != ".md":
...
def determine_skip_reasons(record: ObsidianSyncPassportRecord) -> list[str]:
    """Apply explicit passport safety flags without project-name exceptions."""
...
def build_sync_plan(
    *,
    ...
    if not source_mirror_dir.exists() or not source_mirror_dir.is_dir():
        raise ValueError(
            f"source mirror directory not found for slug '{slug}': {source_mirror_dir}"
        )
    ...
    if eligible:
        for source_file in allowed_files:
            ...
            ensure_destination_within_vault_root(
                destination_path,
                vault_project_root,
            )
            ...
            file_actions.append(
                ObsidianSyncFileAction(
                    ...
                    backup_path=(
                        build_backup_path(destination_path, backup_suffix)
                        if existed_before
                        else None
                    ),
...
def create_sync_plan_from_args(
